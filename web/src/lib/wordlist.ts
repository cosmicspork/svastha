// The EFF "short" wordlist #1 (1296 words, all ≤ 5 letters, verbatim from
// https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt). Embedded
// rather than pulled at runtime so passphrase generation works offline and the
// list is auditable in-tree; it is loaded only on the export path (the recipient
// derives the key from the typed phrase, so it needs no list), so it can be
// lazy-imported and never weighs on the main bundle.
//
// 1296 = 6^4, so each uniformly-drawn word contributes log2(1296) ≈ 10.34 bits.
// A file-share passphrase draws FILE_SHARE_PASSPHRASE_WORDS words (see
// fileShare.ts); at 7 words that is ≈ 72.4 bits, comfortably over the 64-bit
// floor the design sets — which is what makes PBKDF2 an adequate KDF here (the
// entropy is app-generated and known, not a human-chosen low-entropy secret).
