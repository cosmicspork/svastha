...
pub async fn put_blob(
    ...
    state.store.put(&owner.0, &id, body.to_vec());
...
pub async fn get_blob(
    ...
    match state.store.get(&owner.0, &id) {
        Some(blob) => ([(header::CONTENT_TYPE, "application/octet-stream")], blob).into_response(),
...
pub async fn delete_blob(
    ...
    if state.store.delete(&owner.0, &id) {
...
/// A blob id is a short, filesystem-safe token. Enforced now (even though the
/// in-memory store just uses it as a map key) so the contract is already safe
/// against path traversal for the future filesystem backend.
